...
document.getElementById('exportBtn').addEventListener('click', async function() {
    ...
    const mascotPath = (selectedTheme === 'corporate') 
        ? './business-tappy-nobg.png' 
        : './tappy-nobg.svg';

    let activeMascot = "";
    try {
        activeMascot = await getBase64FromUrl(mascotPath);
    } catch (err) {
        console.error("Error while loading mascot:", err);
    }
...
<body style="${themeStyles.body} margin: 0; padding: 24px; display: flex; flex-direction: column; align-items: center;">
    ...
    <div style="${themeStyles.container} padding: 40px; padding-top: 60px; border-radius: 16px; text-align: center; position: relative; max-width: 384px; width: 100%; box-shadow: 0 4px 6px rgba(0,0,0,0.05); box-sizing: border-box;">
        
        <img src="${activeMascot}" alt="Tappy Mascot" style="position: absolute; top: 16px; left: 16px; width: 68px; height: auto;">
...
document.getElementById('themeInput').addEventListener('change', function(e) {
    const mascotImg = document.querySelector('.mascot-card');
    if (e.target.value === 'corporate') {
        mascotImg.src = './business-tappy-nobg.png'; 
    } else {
        mascotImg.src = './tappy-nobg.svg';
    }
});